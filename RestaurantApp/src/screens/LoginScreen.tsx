...
export default function LoginScreen() {
    ...
    const { restaurant, loading: restaurantLoading, error: restaurantError, refetch: refetchRestaurant } = useFetchRestaurant(resId);
    const handleLogin = async (values: { email: string; password: string }) => {
        const { email, password } = values;
        console.log('email:', email);
        setLoading(true);
        try {
            const response = await axios.post(`${API}/api/auth/login/Vendor`, {
                email,
                password,
            });

            const { userToken, username, _id, profile, vefifyVendor } = response.data;
            ...
            dispatch(setUserData({
                ...
                email: email,
                userId: _id,
                verifyVendor: vefifyVendor,
            }));
            setLoading(false);
            // console.log('restaurant:', restaurant?.verification);
            ...
            //     });
            // }
            // else if (restaurant?.verification === "Pending") {
