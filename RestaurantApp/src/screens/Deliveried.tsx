...
const Deliveried: FC<Props> = (props) => {
    const { resId } = useSelector((state: RootState) => state.res);
    console.log('resId', resId);
    const { orders, loading, error, refetch } = useFetchOrders('Deliveried', resId);
                    ...
                    renderItem={({ item, index }) => (
                        <View key={index} style={styles.foodItem}>
                            ...
                            <View style={styles.foodDetails}>
                                ...
                                {item.additives && <Text style={styles.foodNote}>{item.additives?.map((additive) => '+ ' + additive.title).join(', ')}</Text>}
    ...
    return (
        <View style={styles.orderListContainer}>
            <FlatList
                data={Object.entries(groupOrdersByDate(orders))}
                    ...
                    <View style={styles.dateSection}>
                        ...
                        >
                            <Text style={styles.dateSectionText}>
                                {moment(date).format('DD/MM/YYYY')} ({dateOrders.length} đơn)
                            </Text>
                            <Icon
                                name={collapsedSections[ date ] ? 'chevron-down' : 'chevron-up'}
                                size={24}
                                color="#666"
                            />
                        </TouchableOpacity>
                        {!collapsedSections[ date ] && (
                            <View style={styles.dateSectionContent}>
                                {dateOrders.map((order, index) => renderOrderItem({ order }))}
                            </View>
                        )}
                    </View>
                )}
