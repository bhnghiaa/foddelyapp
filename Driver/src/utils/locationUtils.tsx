...
export const calculateDistance = (start: Coordinates, end: Coordinates): number => {
  ...
  // Constants for Earth's dimensions (WGS-84 ellipsoid)
  const EARTH_SEMI_MAJOR_AXIS = 6378137; // meters
  const FLATTENING = 1 / 298.257223563;
  const EARTH_SEMI_MINOR_AXIS = (1 - FLATTENING) * EARTH_SEMI_MAJOR_AXIS;
  ...
  for (let iteration = 0; iteration < MAX_ITERATIONS; iteration++) {
    ...
    cos2α = Math.max(0, 1 - sinα * sinα); // Handle numerical errors
    ...
    if (Math.abs(λ - λʹ) <= CONVERGENCE_THRESHOLD) {
      ...
      return EARTH_SEMI_MINOR_AXIS * A * (σ - Δσ) / 1000; // Return in kilometers
    }
  }
...
export const formatTime = (minutes: number) => {
  ...
  if (hours === 0) return `${Math.ceil(mins)} phút`;
  return `${hours} giờ ${Math.ceil(mins)} phút`;
};
...
export const findOptimalRoute = (
  ...
  // Skip optimization if no routes
  if (!routes.length) {
