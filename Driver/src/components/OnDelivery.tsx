...
const OnDelivery: FC<Props> = (props) => {
    const navigation = useNavigation<NavigationProp<RootStackParamList>>();
    const { orders, loading, error, refetch } = useFetchOrders('Out_for_Delivery');
    const [ refreshing, setRefreshing ] = useState(false);
    const { updateOrderStatus, updatePaymentStatus } = useUpdateOrder();
    ...
    const handleComplete = async (orderId: string) => {
        ...
        await updateOrderStatus(orderId, 'Deliveried');
    ...
    const handleCheckMaps = (restaurant: string, deliveryAddress: string) => {
        navigation.navigate('Location', { address: "", deliveryAddress: deliveryAddress });
    }
        ...
        return (
            <View style={styles.orderCard}>
                ...
                <View style={styles.restaurantRow}>
                    <TouchableOpacity style={[ styles.preparingButton, { backgroundColor: COLORS.red } ]} onPress={() => handleCancel(order._id)}>
                        <Text style={styles.preparingButtonText}>Hủy đơn</Text>
                        <MaterialIcons name="cancel" size={20} color="#fff" />
                    </TouchableOpacity>
                    <TouchableOpacity style={styles.preparingButton} onPress={() => handleComplete(order._id)}>
                        <Text style={styles.preparingButtonText}>Hoàn thành</Text>
